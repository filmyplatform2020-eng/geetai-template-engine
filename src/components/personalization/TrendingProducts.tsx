        ...
        {trending.map((p, i) => (
          <Link key={p.slug} href={`/review/${p.slug}`}>
            <Card className="flex h-full flex-col p-5">
              <span className="mb-2 text-xs text-muted">#{i + 1}</span>
              <p className="mb-1 text-sm font-medium text-primary">{p.product}</p>
              <p className="mb-2 text-xs text-muted">{p.brand}</p>
